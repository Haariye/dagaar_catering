...
function _action(frm, method) {
	// Map method names to (doctype, fieldname on Catering Order that stores the link)
	const targetMap = {
		'create_quotation':         { doctype: 'Quotation',                 link_field: 'quotation' },
		'create_sales_order':       { doctype: 'Sales Order',               link_field: 'sales_order' },
		'create_deposit_payment':   { doctype: 'Payment Entry',             link_field: null },
		'create_cost_sheet':        { doctype: 'Catering Cost Sheet',       link_field: 'cost_sheet' },
		'create_production_plan':   { doctype: 'Catering Production Plan',  link_field: 'production_plan' },
		'create_material_request':  { doctype: 'Material Request',          link_field: 'material_request' },
		'create_sales_invoice':     { doctype: 'Sales Invoice',             link_field: 'sales_invoice' },
		'create_delivery_plan':     { doctype: 'Catering Delivery Plan',    link_field: 'delivery_plan' },
		'create_closing_sheet':     { doctype: 'Catering Closing Sheet',    link_field: 'closing_sheet' },
		'close_catering_order':     { doctype: null,                        link_field: null },
	};
	...
	frappe.call({
		...
		callback: (r) => {
			if (r.message && target.doctype) {
				// r.message is the name of the newly created document
				const newDocName = r.message;
				frappe.show_alert({
					message: __('Created {0} {1}', [target.doctype, newDocName]),
					indicator: 'green'
				}, 5);
				// Redirect to the newly created document
				frappe.set_route('Form', target.doctype, newDocName);
			} else if (r.message) {
				// Generic action like close_catering_order — just reload
				frm.reload_doc();
			}
		},
		error: (xhr) => {
			frappe.msgprint({
				...
				message: __('Could not complete the action. Check console for details.'),
				indicator: 'red'
			});
		}
	});
...
// ════════════════════════════════════════════════════════════════════════════
// BYPASS DEPOSIT — only visible to managers
// ════════════════════════════════════════════════════════════════════════════
...
function _setup_bypass_visibility(frm) {
	...
	// Hide the bypass_deposit field if user is not a manager
	if (frm.fields_dict.bypass_deposit) {
		frm.toggle_display('bypass_deposit', is_manager);
	}

	// If bypass is enabled, show a clear visual indicator
