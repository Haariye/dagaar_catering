...
frappe.ui.form.on('Catering Order Menu Package', {
	...
	menu_packages_remove: function(frm) {
		_schedule_auto_reload(frm);
	},
});
...
function _schedule_auto_reload(frm) {
	if (frm._auto_reload_timer) clearTimeout(frm._auto_reload_timer);
	frm._auto_reload_timer = setTimeout(() => {
		if (frm.is_new()) return;
		if (frm.doc.production_plan) return;
		if (!(frm.doc.menu_packages && frm.doc.menu_packages.length)) return;

		// Save first if dirty, then reload items
		...
		doSave.then(() => {
			frappe.call({
				...
				freeze: true,
				freeze_message: __('Updating items from packages...'),
				callback: () => {
					frm.reload_doc();
					frappe.show_alert({
						message: __('Items updated to match packages'), indicator: 'green'
					}, 3);
				},
			});
		});
...
function _setup_action_buttons(frm) {
	...
	// Void Catering Order (manager only, before production)
	if (d.docstatus === 1 && _is_manager_js() &&
	    !d.production_plan &&
	    !['Closed', 'Cancelled', 'Void'].includes(d.status)) {
		frm.add_custom_button(__('⛔ Void Order'), () => {
			frappe.prompt({
				fieldname: 'reason', label: __('Reason for voiding (optional)'),
				fieldtype: 'Small Text',
			}, (values) => {
				frappe.confirm(
					__('Void this order? It will be disabled but kept in records. Linked documents will NOT be cancelled.'),
					() => {
						frappe.call({
...
function _open_in_new_tab(frm, doctype, docname) {
	// Silent flow: don't navigate, just show a popup with the new doc name
	// and Print / Close buttons. The form reloads so linked-doc fields update.
	const dialog = new frappe.ui.Dialog({
	...
	// Add a "Close" button as a tertiary action via the modal footer
	dialog.show();

	// Reload the parent form so its sales_order / sales_invoice fields update
