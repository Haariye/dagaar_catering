...
frappe.ui.form.on('Catering Order Menu Package', {
	...
	guest_count: function(frm, cdt, cdn) {
		...
		// Find linked items (items.menu_package_item references this row's name)
		const linked_items = (frm.doc.items || []).filter(
			it => it.menu_package_item === row.name
		);

		if (linked_items.length > 0 && row.guest_count) {
			// Auto-update linked items to the new guest count
			linked_items.forEach(it => {
			...
			frappe.show_alert({
				message: __('Updated {0} linked item(s) to {1} guests', [linked_items.length, row.guest_count]),
				indicator: 'blue'
			}, 4);
		}
	},
