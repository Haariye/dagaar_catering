#!/bin/bash
# DagaarSoft Catering — Deploy Fix Script
# Run this from the frappe-bench directory: cd ~/frappe-bench && bash deploy_fix.sh

set -e

TARGET=apps/dagaar_catering/dagaar_catering/catering_management/controllers/catering_order.py

if [ ! -f "$TARGET" ]; then
    echo "ERROR: $TARGET not found. Are you in ~/frappe-bench?"
    exit 1
fi
...
echo "Writing new catering_order.py..."
...
COUNT=$(grep -c "_safe_sum_with_catering_link" "$TARGET")
if [ "$COUNT" -lt 5 ]; then
    echo "ERROR: file write failed (only $COUNT references found)"
    exit 1
fi
echo "  ✓ $COUNT references to _safe_sum_with_catering_link found"

echo ""
echo "Clearing cache and restarting..."
bench --site uat.dagaartech.com clear-cache
sudo supervisorctl restart frappe-bench-web: frappe-bench-workers: 2>/dev/null || bench restart

echo ""
echo "✅ DONE. Hit Ctrl+Shift+R in your browser to reload."
