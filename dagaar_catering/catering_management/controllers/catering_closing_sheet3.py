...
def _check_outstanding(doc):
	"""Block close if invoice has outstanding amount, unless bypassed by manager."""
	co_data = frappe.db.get_value("Catering Order", doc.catering_order,
		["sales_invoice", "total_paid"], as_dict=True) or {}
	...
	if outstanding > 0 and not doc.get("bypass_outstanding"):
		is_mgr = any(r in frappe.get_roles() for r in
			("Catering Manager", "Catering Management", "System Manager", "Administrator"))
		msg = _("Cannot close order: customer has an outstanding balance of {0} {1}.").format(
			outstanding, frappe.db.get_value("Catering Order", doc.catering_order, "currency"))
		if is_mgr:
			msg += " " + _("As a Catering Manager you can tick 'Bypass Outstanding Balance' on this Closing Sheet to override.")
		else:
			msg += " " + _("Ask a Catering Manager to bypass, or collect the remaining payment first.")
