...
def refresh_cost_sheet(catering_order):
	"""Force-refresh the live Cost Sheet for this Catering Order.

	Called by various linker hooks (Wastage on_submit, Stock Entry on_submit,
	Journal Entry on_submit, etc.) so the Cost Sheet stays current without
	user intervention.
	"""
	...
	try:
		...
		# Re-run validate (which recomputes) and save — works on both Draft and
		# Submitted Cost Sheets because all cost fields have allow_on_submit=1
		cs.save(ignore_permissions=True)
		return cs.name
	except Exception as e:
		frappe.log_error(f"refresh_cost_sheet({catering_order}) failed: {str(e)[:300]}",
			"Cost Sheet Auto-Refresh")
		return None
