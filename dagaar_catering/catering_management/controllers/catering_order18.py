...
def _check_for_rebilling_required(doc):
	"""Set doc.requires_rebill = 1 if the order changed AFTER a SI was created.

	Compares current snapshot to last_billed_snapshot. If different, the SI
	needs re-syncing — on_update will call update_sales_invoice automatically.
	"""
	...
	if not last:
		# First time we see this — store snapshot, no rebill
