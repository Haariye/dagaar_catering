...
def refresh_cost_sheet_on_event(doc, method=None):
	"""Generic linker: when an upstream cost-relevant doc is submitted,
	refresh the Cost Sheet attached to its catering_order so figures stay live.

	Called from hooks for: Catering Wastage Entry, Catering Emergency Expense,
	Stock Entry, Journal Entry (Quick Expense), Purchase Invoice.
	"""
	catering_order = getattr(doc, "catering_order", None)
	if not catering_order:
		return
	try:
		...
		frappe.log_error(f"refresh_cost_sheet_on_event failed: {str(e)[:200]}",
			"Catering Cost Sheet Refresh")
