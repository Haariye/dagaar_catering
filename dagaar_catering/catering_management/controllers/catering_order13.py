...
@frappe.whitelist()
def create_quick_expense(catering_order, payee, expense_account, paid_from_account,
                          amount, expense_date, memo=None, reference_no=None):
	"""Create a Journal Entry for a quick expense linked to this Catering Order / Project.

	Mimics QuickBooks 'Write Check' form:
	  Payee, Pay From (Bank/Cash), Expense Account, Date, Amount, Memo, Reference No.

	Creates a JE: Dr Expense Account, Cr Bank/Cash. Tagged with the project and catering_order.
	"""
	co = frappe.get_doc("Catering Order", catering_order)
	if co.docstatus != 1:
	...
	if not paid_from_account:
		frappe.throw(_("Pay From Account is required."))

	je = frappe.new_doc("Journal Entry")
	je.voucher_type = "Bank Entry" if "bank" in (paid_from_account or "").lower() else "Cash Entry"
	...
	je.user_remark = f"Quick Expense for {co.name}: {memo or payee}"
	...
	je.append("accounts", {
		...
		"cost_center": co.cost_center,
		"project": co.project,
		"party_type": "Supplier" if frappe.db.exists("Supplier", payee) else None,
		"party": payee if frappe.db.exists("Supplier", payee) else None,
		"user_remark": memo or f"Expense - {payee}",
	})

	# Credit the pay-from account
	je.append("accounts", {
		"account": paid_from_account,
		"credit_in_account_currency": amount,
		"cost_center": co.cost_center,
		"project": co.project,
		"user_remark": f"Paid {payee}",
	})
	...
	_log_activity(co, "Document Created", f"Quick Expense JE {je.name} for {payee}: {amount}",
