...
function _setup_action_buttons(frm) {
	...
	// ──────────────────────────────────────────────────────────────────
	// APPROVAL WORKFLOW BUTTONS (role-based)
	// ──────────────────────────────────────────────────────────────────
	_setup_approval_buttons(frm);

	// Operational buttons only appear when order is APPROVED
...
const _MANAGER_ROLES = ['Catering Manager', 'Catering Management', 'System Manager', 'Administrator'];

function _is_catering_manager() {
	const user_roles = frappe.user_roles || [];
	return _MANAGER_ROLES.some(r => user_roles.includes(r));
}

function _setup_approval_buttons(frm) {
	const status = frm.doc.approval_status || 'Draft';
	const is_mgr = _is_catering_manager();

	let color = 'gray';
	let label = status;
	if (status === 'Pending Approval') { color = 'orange'; label = '⏳ Pending Approval'; }
	else if (status === 'Approved') { color = 'green'; label = '✅ Approved'; }
	else if (status === 'Rejected') { color = 'red'; label = '❌ Rejected'; }
	else { color = 'gray'; label = '📝 Draft'; }
	frm.dashboard.set_headline_alert(`<strong>Approval:</strong> ${label}`, color);

	if (status === 'Draft') {
		frm.add_custom_button(__('Submit for Approval'), () => {
			frappe.confirm(__('Submit this order for Manager review?'), () => {
				frappe.call({
					method: 'dagaar_catering.catering_management.controllers.catering_order.submit_for_approval',
					args: { catering_order: frm.doc.name },
					callback: () => frm.reload_doc(),
				});
			});
		}, __('Approval')).addClass('btn-warning');
	}

	if (status === 'Pending Approval' && is_mgr) {
		frm.add_custom_button(__('Approve'), () => {
			frappe.confirm(__('Approve this Catering Order?'), () => {
				frappe.call({
					method: 'dagaar_catering.catering_management.controllers.catering_order.approve_order',
					args: { catering_order: frm.doc.name },
					callback: () => frm.reload_doc(),
				});
			});
		}, __('Approval')).addClass('btn-success');
		...
		frm.add_custom_button(__('Reject'), () => {
			frappe.prompt({
				...
				frappe.call({
					method: 'dagaar_catering.catering_management.controllers.catering_order.reject_order',
					args: { catering_order: frm.doc.name, reason: values.reason || '' },
					callback: () => frm.reload_doc(),
				});
			}, __('Reject Order'), __('Reject'));
		}, __('Approval')).addClass('btn-danger');
	}

	if (status === 'Rejected') {
		if (is_mgr) {
			frm.add_custom_button(__('Reopen (Undo Reject)'), () => {
				frappe.confirm(__('Reopen this Rejected order? It will return to Draft state.'), () => {
					frappe.call({
						method: 'dagaar_catering.catering_management.controllers.catering_order.reopen_order',
						args: { catering_order: frm.doc.name },
						callback: () => frm.reload_doc(),
					});
				});
		...
		frm.add_custom_button(__('Revise & Resubmit'), () => {
			frappe.confirm(__('Mark this as Draft so you can revise it?'), () => {
				frappe.call({
					method: 'dagaar_catering.catering_management.controllers.catering_order.reopen_order',
					args: { catering_order: frm.doc.name },
					callback: () => frm.reload_doc(),
	...
	if (status === 'Pending Approval' || (status === 'Approved' && !is_mgr)) {
		frm.set_read_only(true);
		if (status === 'Pending Approval') {
			frm.dashboard.add_indicator(__('Locked: awaiting approval'), 'orange');
		} else if (status === 'Approved' && !is_mgr) {
			frm.dashboard.add_indicator(__('Locked: approved order (Manager-only edit)'), 'green');
		}
	}
}
