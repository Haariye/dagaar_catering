...
function _setup_action_buttons(frm) {
	...
	// Sales Invoice button — visible whenever there's unbilled amount.
	// Driven by money math (order_total vs net_billed), not just by
	// presence/absence of d.sales_invoice. Handles initial + amended cases.
	if (d.sales_order) {
		frappe.call({
			...
			callback: (br) => {
				const bs = br.message || {};
				if (bs.show_sales_invoice_btn) {
					const label = bs.has_sales_invoice
						? __('Sales Invoice') + ` <span style="color:#c0392b;">(${format_currency(bs.unbilled_amount, bs.currency)} unbilled)</span>`
						: __('Sales Invoice');
	...
	// Regenerate Bill — money-driven; shows when an SI exists and totals differ
	if (d.sales_invoice) {
		frappe.call({
			method: 'dagaar_catering.catering_management.controllers.catering_order.get_billing_status',
			args: { catering_order: d.name },
