...
def on_update(doc, method=None):
	"""Auto-sync the Sales Invoice when items/guests changed after submit.

	Fires when requires_rebill flag is set on a submitted order that has a SI.
	Calls update_sales_invoice (the same function the button calls), which
	handles draft-edit vs submitted-supplementary/credit-note logic.
	"""
	if doc.docstatus != 1:
		return
	if not doc.sales_invoice:
		return
	if not doc.requires_rebill:
		return
	if getattr(doc.flags, "in_auto_rebill", False):
		return  # Reentrancy guard

	try:
		doc.flags.in_auto_rebill = True
		update_sales_invoice(doc.name)
	except Exception as e:
		frappe.log_error(f"Auto-rebill failed for {doc.name}: {str(e)[:300]}",
			"Catering Order Auto-Rebill")
	finally:
		doc.flags.in_auto_rebill = False
...
def _validate_guests(doc):
	"""Validate guest counts against package min limits.

	Each Menu Package row's guest_count must be >= package.min_guests
	(if min_guests is set). Maximum is NOT enforced — packages can exceed.

	Blocks save with a throw, not just a warning, so the order cannot be
	saved with under-strength guest counts.
	"""
	if flt(doc.total_guests) <= 0 and not (doc.get("menu_packages") or []):
		frappe.throw(_("Total Guests must be greater than 0. Add menu packages with guest counts."))
	...
	for pkg_row in (doc.get("menu_packages") or []):
		...
		try:
			...
			if pkg.min_guests and gc < flt(pkg.min_guests):
				errors.append(
					_("Package <b>{0}</b>: guest count <b>{1}</b> is below the minimum required <b>{2}</b>").format(
						pkg.package_name or pkg_row.menu_package, int(gc), int(pkg.min_guests)
					)
				)
		except Exception:
			continue
	...
	if errors:
		frappe.throw(
			...
			_("Increase the guest counts or remove the package."),
			title=_("Minimum Guest Count Not Met")
		)
...
def void_catering_order(catering_order, reason=None):
	"""Manually void a Catering Order.

	Voiding is different from cancelling:
	  - Cancel: docstatus → 2, cascades cancellations on linked docs
	  - Void: status → 'Void', the order is disabled but linked docs are
	          left alone (quotation may still exist for record-keeping).
	          Used when a quotation went unanswered or the customer backed out
	          before any operational docs were created.

	A voided order cannot have new docs created against it. It is not
	cancelled at the docstatus level, so it stays in the audit trail.
	"""
	if not _is_manager():
	...
	if co.status in ("Closed", "Cancelled", "Void"):
		frappe.throw(_("Order is already {0} and cannot be voided again.").format(co.status))

	# Don't allow voiding past Sales Order Created (it has financial implications)
	if co.production_plan:
		frappe.throw(_("Cannot void an order that has a Production Plan. Use Cancel instead."))

	frappe.db.set_value("Catering Order", catering_order, {
		"status": "Void",
	}, update_modified=True)

	_log_activity(co, "Status Change",
		f"Order voided by {frappe.session.user}" + (f" — Reason: {reason}" if reason else ""))
