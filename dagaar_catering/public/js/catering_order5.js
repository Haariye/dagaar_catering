...
function _setup_action_buttons(frm) {
	...
	// 9. Regenerate Bill — show when order changed after billing
	if (d.requires_rebill) {
		frm.add_custom_button(__('⚠️ Regenerate Bill'), () => {
			frappe.confirm(
				__('This will CANCEL the existing Sales Order and Sales Invoice, then create new ones with the updated guest count and items. Existing payments will reconcile against the new invoice. Continue?'),
				() => {
					frappe.call({
						method: 'dagaar_catering.catering_management.controllers.catering_order.regenerate_bill',
						args: { catering_order: frm.doc.name },
						freeze: true,
						freeze_message: __('Cancelling old documents and creating new ones...'),
						callback: (r) => {
							if (r.message) {
								frappe.show_alert({
									message: __('Rebilled — new SO: {0}, new SI: {1}',
										[r.message.sales_order, r.message.sales_invoice]),
									indicator: 'green'
								}, 8);
								frm.reload_doc();
							}
