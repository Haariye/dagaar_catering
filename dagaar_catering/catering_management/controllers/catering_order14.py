...
def _stock_consumption(catering_order, categories):
	"""Sum the value of raw-material consumption from Stock Entries tagged
	with this Catering Order, bucketed by the production_item's category
	(or the consumed item's category if it's a direct Material Issue).

	For purpose='Manufacture' SEs: the consumed items are raw materials.
	We identify which Catering Order Item the SE produced via Work Order's
	production_item, then look up that item's category.
	"""
	try:
		# Build item_code → category map from Catering Order Item
		cat_map = {}
		...
		""", catering_order, as_dict=True):
			if r.item_code and r.category:
				cat_map[r.item_code] = r.category

		if not cat_map:
			return 0

		# Pull all consumption rows
		rows = frappe.db.sql("""
			SELECT se.work_order, sle.item_code, ABS(sle.stock_value_difference) AS amt
		...
		for row in rows:
			cat = None
			# (a) SLE item itself is a Catering Order Item
			if row.item_code in cat_map:
				cat = cat_map[row.item_code]
			# (b) Look up via Work Order's production_item
			elif row.work_order:
				if row.work_order not in wo_cat_cache:
					try:
						prod = frappe.db.get_value("Work Order", row.work_order, "production_item")
						wo_cat_cache[row.work_order] = cat_map.get(prod)
					except Exception:
						wo_cat_cache[row.work_order] = None
				cat = wo_cat_cache.get(row.work_order)

			# (c) Fallback for uncategorized — default to "Food"
			if not cat:
				cat = "Food"
