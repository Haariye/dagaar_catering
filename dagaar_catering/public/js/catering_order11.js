...
function _setup_action_buttons(frm) {
	...
	// Load Items from Packages — works in Draft AND Submitted (so user can refresh
	// items after changing guest counts post-submit). Hidden when order is Closed.
	if (d.status !== 'Closed' && d.menu_packages && d.menu_packages.length > 0) {
		const label = d.docstatus === 1
			? '🍽 Refresh Items from Packages'
			: '🍽 Load Items from Packages';
